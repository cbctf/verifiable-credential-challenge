...
- Do you even need a local ledger? If not, you can use a public sandbox ledger,
  ...
  when running the holder-itpss demo in the [demo](demo) folder, you can run (for
  example), the itpss agent using the command:
...
- Start the tails server as usual:
  - `./manage start --logs`
- To run the scenario manually, start itpss and let the agent know it needs to TAA-accept before doing any ledger writes:
  - `./run_demo itpss --revocation --taa-accept`, and then you can run through all the transactions using the Swagger page.
