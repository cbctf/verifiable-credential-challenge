...
class holderAgent(BaseAgent):
    def __init__(self, port: int, **kwargs):
        super().__init__("holder", port, seed=None, **kwargs)
        self.extra_args = [
            "--auto-accept-invites",
        ...
        self.timing_log = "logs/holder_perf.log"
...
class itpssAgent(BaseAgent):
    def __init__(self, port: int, **kwargs):
        super().__init__("itpss", port, seed="random", **kwargs)
        self.extra_args = [
            "--auto-accept-invites",
            "--auto-accept-requests",
            "--monitor-ping",
    ...
    async def publish_defs(self, support_revocation: bool = False):
        ...
        schema_body = {
            "schema_name": "Certificate of Education",
...
async def main(
    ...
    holder = None
    itpss = None
    holder_mediator_agent = None
    itpss_mediator_agent = None
    ...
    try:
        ...
        await holder.listen_webhooks(start_port + 2)
        ...
        await itpss.listen_webhooks(start_port + 5)
        await itpss.register_did()

        with log_timer("Startup duration:"):
            await holder.start_process()
            await itpss.start_process()

            if mediation:
                holder_mediator_agent = await start_mediator_agent(
                    start_port + 8, genesis, multi_ledger_config_path
                )
                if not holder_mediator_agent:
                    raise Exception("Mediator agent returns None :-(")
                itpss_mediator_agent = await start_mediator_agent(
                    start_port + 11, genesis, multi_ledger_config_path
                )
                if not itpss_mediator_agent:
                    raise Exception("Mediator agent returns None :-(")
            else:
                holder_mediator_agent = None
                itpss_mediator_agent = None
        ...
        with log_timer("Connect duration:"):
            if multitenant:
                ...
                await holder.register_or_switch_wallet(
                    "holder.initial",
                    webhook_port=None,
                    mediator_agent=holder_mediator_agent,
                )
                await itpss.register_or_switch_wallet(
                    "itpss.initial",
                    public_did=True,
                    webhook_port=None,
                    mediator_agent=itpss_mediator_agent,
                )
            elif mediation:
                # we need to pre-connect the agent(s) to their mediator (use the same
                # mediator for both)
                if not await connect_wallet_to_mediator(holder, holder_mediator_agent):
                    log_msg("Mediation setup FAILED :-(")
                    raise Exception("Mediation setup FAILED :-(")
                if not await connect_wallet_to_mediator(itpss, itpss_mediator_agent):
                    log_msg("Mediation setup FAILED :-(")
                    raise Exception("Mediation setup FAILED :-(")

            invite = await itpss.get_invite(use_did_exchange)
            await holder.receive_invite(invite["invitation"])
            await asyncio.wait_for(itpss.detect_connection(), 30)

        if action != "ping":
            with log_timer("Publish duration:"):
                await itpss.publish_defs(revocation)
            # cache the credential definition
            await holder.fetch_credential_definition(itpss.credential_definition_id)

        if show_timing:
            await holder.reset_timing()
            await itpss.reset_timing()
            if mediation:
                await holder_mediator_agent.reset_timing()
                await itpss_mediator_agent.reset_timing()

        semaphore = asyncio.Semaphore(threads)

        def done_propose(fut: asyncio.Task):
            semaphore.release()
            holder.check_task_exception(fut)

        def done_send(fut: asyncio.Task):
            semaphore.release()
            itpss.check_task_exception(fut)
        ...
        async def propose_credential(index: int):
            ...
            asyncio.ensure_future(
                holder.propose_credential(
                    attributes, itpss.credential_definition_id, comment, not revocation
                )
        ...
        async def send_credential(index: int):
            ...
            attributes = test_cred(index)
            asyncio.ensure_future(
                itpss.send_credential(attributes, comment, not revocation)
        ...
        async def send_ping(index: int):
            await semaphore.acquire()
            asyncio.ensure_future(itpss.send_ping(str(index))).add_done_callback(
                done_send
            )
        ...
        if action == "ping":
            recv_timer = itpss.log_timer(f"Completed {issue_count} ping exchanges in")
            batch_timer = itpss.log_timer(f"Started {batch_size} ping exchanges in")
        else:
            recv_timer = itpss.log_timer(
                f"Completed {issue_count} credential exchanges in"
            )
            batch_timer = itpss.log_timer(
        ...
        with progress() as pb:
            ...
            try:
                ...
                issue_task = asyncio.ensure_future(
                    check_received(itpss, issue_count, issue_pg)
                )
                issue_task.add_done_callback(itpss.check_task_exception)
                receive_task = asyncio.ensure_future(
                    check_received(holder, issue_count, receive_pg)
                )
                receive_task.add_done_callback(holder.check_task_exception)
                with itpss.log_timer(completed):
        ...
        itpss.log(f"Average time per {item_long}: {avg:.2f}s ({1/avg:.2f}/s)")

        if holder.postgres:
            await holder.collect_postgres_stats(f"{issue_count} {item_short}s")
            for line in holder.format_postgres_stats():
                holder.log(line)
        if itpss.postgres:
            await itpss.collect_postgres_stats(f"{issue_count} {item_short}s")
            for line in itpss.format_postgres_stats():
                itpss.log(line)

        if revocation and itpss.revocations:
            (rev_reg_id, cred_rev_id) = next(iter(itpss.revocations))
            print(
        ...
        if show_timing:
            timing = await holder.fetch_timing()
            if timing:
                for line in holder.format_timing(timing):
                    holder.log(line)

            timing = await itpss.fetch_timing()
            if timing:
                for line in itpss.format_timing(timing):
                    itpss.log(line)
            if mediation:
                timing = await holder_mediator_agent.fetch_timing()
                if timing:
                    for line in holder_mediator_agent.format_timing(timing):
                        holder_mediator_agent.log(line)
                timing = await itpss_mediator_agent.fetch_timing()
                if timing:
                    for line in itpss_mediator_agent.format_timing(timing):
                        itpss_mediator_agent.log(line)
        ...
        try:
            if itpss:
                await itpss.terminate()
        except Exception:
            LOGGER.exception("Error terminating agent:")
            terminated = False
        try:
            if holder_mediator_agent:
                await holder_mediator_agent.terminate()
            if itpss_mediator_agent:
                await itpss_mediator_agent.terminate()
        except Exception:
            LOGGER.exception("Error terminating agent:")
            terminated = False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Start credential exchange with a credential proposal from holder",
    )
    parser.add_argument(
