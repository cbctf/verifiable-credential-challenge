...
class AriesAgent(DemoAgent):
    ...
    async def handle_present_proof_v2_0(self, message):
        ...
        if state == "request-received":
            ...
            # self.log("hello")
            # req = proof["pres"]["presentations~attach"][0]["data"]
            req = proof["by_format"]["pres"]["indy"]["requested_proof"]
            # log_json(req)

            # message = json.dumps(req)
            message = '{"name": "Zara Laila", "degree": "Maths"}'
            message_bytes = message.encode('ascii')
            base64_bytes = base64.b64encode(message_bytes)
            base64_message = base64_bytes.decode('ascii')
            self.log("Presented Proof", base64_message)
    ...
    async def handle_basicmessages(self, message):
        ...
        ok = message["content"]
        oklow = ok.lower()
        var1 = sha256(oklow.encode('utf-8')).hexdigest()
        #self.log(var1)


        message1 = "5bda0a111f636e9ada51448e8f43722ddf314a533b0a1a1ae31ca900ac61a0c6"
        message2 = "6ceed66c5130674bd2763ca051956124dab919027327d24c6504a5cf8eebfdb0"

        if (var1 == message1):
            message5 = 'e0NURl9WRVJJRklBQkxFX0NSRURFTlRJQUxfQ0hBTExFTkdFX0ZVTl8xfQ=='
            base64_message1 = base64.b64decode(message5)
            self.log(base64_message1)
        elif (var1 == message2):
            message6 = 'e0NURl9WRVJJRklBQkxFX0NSRURFTlRJQUxfV0hBVFNfTVlfTkFNRX0='
            base64_message2 = base64.b64decode(message6)
            self.log(base64_message2)
    ...
    async def generate_invitation(
        ...
        with log_timer("Generate invitation duration:"):
            ...
            log_status(
                "#7 Create a connection to holder and print out the invite details"
            )
        ...
        if display_qr:
            ...
            log_msg(
                json.dumps(invi_rec["invitation"]), label="Invitation details:", color=None
            )
...
def arg_parser(ident: str = None, port: int = 8020):
    ...
    "ident", if specified, refers to one of the standard demo personas - holder, itpss, acme or performance.
    """
    ...
    if (not ident) or (ident != "holder"):
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Reuse connections by using itpss public key in the invite. "
...
async def test_main(
    ...
    itpss_container = None
    holder_container = None
    try:
        # initialize the containers
        itpss_container = AgentContainer(
            genesis_txns=genesis,
            ident="itpss.agent",
        ...
        holder_container = AgentContainer(
            genesis_txns=genesis,
            ident="holder.agent",
        ...
        # start the agents - itpss gets a public DID and schema/cred def
        await itpss_container.initialize(
        ...
        await holder_container.initialize()

        # itpss create invitation
        invite = await itpss_container.generate_invitation()

        # holder accept invitation
        invite_details = invite["invitation"]
        connection = await holder_container.input_invitation(invite_details)

        # wait for itpss connection to activate
        await itpss_container.detect_connection()
        await holder_container.detect_connection()

        # TODO itpss issue credential to holder
        # TODO holder check for received credential
        ...
        try:
            ...
            if holder_container:
                log_msg("Shutting down holder agent ...")
                await holder_container.terminate()
            if itpss_container:
                log_msg("Shutting down itpss agent ...")
                await itpss_container.terminate()
        except Exception as e:
