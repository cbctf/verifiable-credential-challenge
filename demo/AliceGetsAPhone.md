# holder Gets a Mobile Agent! <!-- omit in toc -->

In this demo, we'll again use our familiar itpss ACA-Py agent to issue credentials to holder, but this time holder will use a mobile wallet. To do this we need to run the itpss agent on a publicly accessible port, and holder will need a compatible mobile wallet. We'll provide pointers to where you can get them.
...
- [Run `itpss` With Extra Parameters](#run-itpss-with-extra-parameters)
...
We'll come back to this in a minute, when we start the `itpss` agent!

There are a couple of extra steps you need to take to prepare to run the itpss agent locally:
...
Note that with _Play with Docker_ it can be challenging to capture the information you need from the log file as it scrolls by, you can try leaving off the `--events` option when you run the itpss agent to reduce the quantity of information logged to the screen.

### Run `itpss` With Extra Parameters
...
```bash
TAILS_NETWORK=docker_tails-server LEDGER_URL=http://test.bcovrin.vonx.io ./run_demo itpss --aip 10 --revocation --events
```

(Note that we have to start itpss with `--aip 10` for compatibility with mobile clients.)
...
```bash
PUBLIC_TAILS_URL=https://c4f7fbb85911.ngrok.io LEDGER_URL=http://test.bcovrin.vonx.io ./run_demo itpss --aip 10 --revocation --events
```
...
*Note - you may want to leave off the `--events` option when you run the itpss agent, if you are finding you are getting too much logging output.

#### Waiting for the itpss agent to start ...

The `Preparing agent image...` step on the first run takes a bit of time, so while we wait, let's look at the details of the commands. Running itpss is similar to the instructions in the [Aries OpenAPI Demo](./AriesOpenAPIDemo.md) "Play with Docker" section, except:
...
When the itpss agent starts up it automatically creates an invitation and generates a QR code on the screen. On your mobile app, select "SCAN CODE" (or equivalent) and point your camera at the generated QR code. The mobile agent should automatically capture the code and ask you to confirm the connection. Confirm it.
...
We will use the itpss console to issue a credential. This could be done using the Swagger API as we have done in the connection process. We'll leave that as an exercise to the user.

In the itpss console, select option `1` to send a credential to the mobile agent.
...
The itpss agent outputs details to the console; e.g.,

```text
itpss      | Credential: state = credential-issued, cred_ex_id = ba3089d6-92da-4cb7-9062-7f24066b2a2a
itpss      | Revocation registry ID: CMqNjZ8e59jDuBYcquce4D:4:CMqNjZ8e59jDuBYcquce4D:3:CL:50:itpss.agent.degree_schema:CL_ACCUM:4f4fb2e4-3a59-45b1-8921-578d005a7ff6
itpss      | Credential revocation ID: 1
itpss      | Credential: state = done, cred_ex_id = ba3089d6-92da-4cb7-9062-7f24066b2a2a
```
...
We will use the itpss console to ask mobile agent for a proof. This could be done using the Swagger API, but we'll leave that as an exercise to the user.

In the itpss console, select option `2` to send a proof request to the mobile agent.
...
The presentation (proof) request should automatically show up in the mobile agent. Follow the instructions provided by the mobile agent to prepare and send the proof back to itpss. That will look something like this:
...
The process should "just work" for the non-revocation use case. If you are using revocation, your results may vary. As of writing this, we get failures on the wallet side with some mobile wallets, and on the itpss side with others (an error in the Indy SDK). As the results improve, we'll update this. Please let us know through GitHub issues if you have any problems running this.

## Review the Proof

In the itpss console window, the proof should be received as validated.
...
If you have enabled revocation, you can try revoking the credential and publishing its pending revoked status (`itpss` options `5` and `6`). For the revocation step, You will need the revocation registry identifier and the credential revocation identifier (which is 1 for the first credential you issued), as the itpss agent logged them to the console at credential issue.
...
This is supported in the itpss demo, however note that it will only work when running itpss on the Docker playground service [Play with Docker](https://labs.play-with-docker.com/).  (This is because both the itpss agent *and* controller both need to be exposed to the mobile agent.)

If you have gone through the above steps, you can delete the itpss connection in your mobile agent (however *do not* delete the credential that itpss issued to you).

Then in the itpss demo, select option `2a` - itpss will display a QR code which you can scan with your mobile agent.  You will see the same proof request displayed in your mobile agent, which you can respond to.

Behind the scenes, the itpss controller delivers the proof request information (linked from the url encoded in the QR code) directly to your mobile agent, without establishing and agent-to-agent connection first.  If you are interested in the underlying mechanics, you can review the `itpss.py` code in the repository.

## Conclusion

That’s the itpss-Mobile holder demo. Feel free to play with the Swagger API and experiment further and figure out what an instance of a controller has to do to make things work.
