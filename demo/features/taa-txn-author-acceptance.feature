...
# you can run von-network as `./manage start --taa-sample --logs`
   ...
   Scenario Outline: Revoke credential using a ledger with TAA required
      ...
      And "itpss" connects to a ledger that requires acceptance of the TAA
      And "itpss" accepts the TAA
      And "itpss" and "Bob" have an existing connection
      And "Bob" has an issued <Schema_name> credential <Credential_data> from "<issuer>"
      And "itpss" revokes the credential
      And "itpss" successfully revoked the credential
      ...
      Examples:
         ...
         | itpss  | --taa-accept --revocation --public-did   |                  | driverslicense_v2 | Data_DL_MaxValues | DL_age_over_19_v2 |
   ...
   Scenario Outline: Fail to publish revoked credential using a ledger with TAA required, and fix the ledger
      ...
      And "itpss" revokes the credential
      And "itpss" successfully revoked the credential
      When "itpss" rejects the TAA
      And "Bob" has an issued <Schema_name> credential <Credential_data> from "<issuer>"
      And "itpss" attempts to revoke the credential
      And "itpss" fails to publish the credential revocation
      Then "itpss" accepts the TAA
      And "itpss" posts a revocation correction to the ledger
      And "itpss" successfully revoked the credential
      And "Bob" has an issued <Schema_name> credential <Credential_data> from "<issuer>"
      And "itpss" revokes the credential
      And "itpss" successfully revoked the credential
